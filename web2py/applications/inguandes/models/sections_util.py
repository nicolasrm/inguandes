...
def get_sections(db, courseid=None):    
    ...
    for s in sections:
        ...
        section['students'] = db((db.user_section.section==s.section.id) & (db.user_section.the_role==1)).count()
        section['assistants'] = db((db.user_section.section==s.section.id) & (db.user_section.the_role==2)).count()
